...
export default function MeusIngressosPage() {
  ...
  function handleShare(qrCode: string, id: string) {
    const shareUrl = `${window.location.origin}/portaria?code=${qrCode}`;
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-xs font-mono text-zinc-400">
                      Assento: <strong className="text-white">{t.seat || 'Geral'}</strong>
                    </span>
                  </div>

                  <h2 className="text-lg font-bold text-white">{t.event?.title || 'Evento'}</h2>
                  <p className="text-xs text-zinc-400 mt-1">📍 {t.event?.location || 'Local a definir'}</p>
